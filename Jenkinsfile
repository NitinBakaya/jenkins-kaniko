       ...
       script {
              ...
              ])
              {
               sh '''
                  ...
                  git clone https://oauth2:${REPO_PRIVATE_TOKEN}@github.com/NitinBakaya/jenkins-kaniko.git
                  ls
        ...
        container(name: 'kaniko', shell: '/busybox/sh') {
          sh '''#!/busybox/sh
            ...
            /kaniko/executor --context `pwd` --destination nitinbakaya/kaniko:${BUILD_NUMBER}
          '''
        }
       ...
       script {
              ...
              ])
              {
               sh '''
                  ...
                  sudo ./aws/install
                  aws --version
                  aws eks update-kubeconfig --name demo-eks --region ${AWS_DEFAULT_REGION}
                  ...
                  helm upgrade --install hello -f charts/hello/values.yaml --set image.repository=nitinbakaya/kaniko:${BUILD_NUMBER} --namespace hello charts/hello/ --atomic --timeout 1m25s --cleanup-on-fail
               '''
              }
            }
